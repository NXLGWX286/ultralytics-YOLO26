...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True, small_obj_opt=False):
...
class Focus(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, small_obj_opt=False):
        super().__init__()
        self.small_obj_opt = small_obj_opt
        conv_in = c1 * 2 if small_obj_opt else c1 * 4
        self.conv = Conv(conv_in, c2, k, s, p, g, act=act)

    def forward(self, x):
        if self.small_obj_opt:
            return self.conv(torch.cat((x[..., ::2, :], x[..., 1::2, :]), 1))
        else:
